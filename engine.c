...
{
    printf("Sending request to supervisor...\n");
...
static int cmd_start(int argc, char *argv[])
{
    ...
    printf("Starting real container...\n");
...
pid_t pid = clone(child_fn, stack_top, SIGCHLD, &config);
...
strncpy(c->id, argv[2], CONTAINER_ID_LEN);
c->host_pid = pid;
...
if (strcmp(argv[1], "test") == 0) {
    ...
    child_config_t config;
    strcpy(config.id, "test_container");
    strcpy(config.rootfs, "rootfs-alpha");
    ...
    printf("Container process finished\n");
    return 0;
}
